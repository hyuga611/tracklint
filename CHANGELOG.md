...
carrylint で3回続けて出た誤検知の形——**テキスト規則は「言及」を見ていて「実行」を
見ていない**——をこちらにも当てた。コメントが両方向に効いていた。

### コメントは書いてあるだけで、実行されない

計測基盤の有無もコンバージョン呼び出しの有無も、生テキストへの正規表現で見ていた。
...
この1行があるだけで「このプロジェクトは計測している」と判定され、配線系ルールが
全部有効になる。同一のフォームが `clean` から `1 error` に変わっていた。
**「うちは計測していない」と明記した人だけ**が対象になる。
...
これがコンバージョン呼び出しとして数えられ、本当に success 時の計測が無い AJAX
フォームが `ajax-no-conversion` を出さなくなっていた。

- `stripComments()` を追加し、HTML コメントと `<script>` 本文の JS コメントを
  **長さと改行を保ったまま**空白化してから判定する（行番号がずれない）。
- 文字列中の `//` は残す。`https://` も `src="//cdn..."` も**コメントではない**。
  ここで消しすぎると計測基盤を見落として、**配線ルールが黙る方向**に壊れる——
  誤検知より質の悪い壊れ方なので、消す側を保守的にした。
- 単一ファイル判定（`scan`）は HTML コメントだけ落としていたので、そちらも JS
  コメントまで見るようにした。CLI 側（プロジェクト全体の計測基盤判定）は
  コメントを一切落としていなかった。
